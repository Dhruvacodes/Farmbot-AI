...
The dashboard now fills these stream URLs automatically from the Jetson telemetry connection:
...
The publisher auto-detects the Jetson IP used to reach the PC dashboard and publishes both camera stream URLs. If auto-detection picks the wrong network adapter, pass `--camera-host JETSON_IP`.
...
If `streams` is omitted or contains empty values, the dashboard derives both stream URLs from the Jetson's source IP and port `8090`.
